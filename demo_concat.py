...
class Test_Reader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # pdb.set_trace()
        # audio_path = audio_path.replace('/home/nas/DB/AI_grand_challenge_2020/jungwook_test/chochangi_2020audio//enhance_wav', '../t3_audio')
        ...
        if audio_enhanced.sum() == 0.0 or len(time) == 0 or time[0] == -1:
            return LR, audio_path, LR
            ...
            feature_list = []
            # try:
            # print(len(audio_enhanced))
            for i in range(len(audio_enhanced)):
                enhanced, indices = self.vad(audio_enhanced[i])
                feature_enhanced = self.LogMelGccExtractor(enhanced)
                feature_noisy = self.LogMelGccExtractor(audio_noisy[i][:,indices])
                feature_list.append(torch.FloatTensor(np.concatenate([feature_enhanced, feature_noisy], axis=0)))
            # except: 
            #     pdb.set_trace()
            '''(channels, seq_len, mel_bins)'''
            return feature_list, audio_path, LR
...
# CKPT_PATH = '/home/nas/user/minseok/FINAL_CONCAT/2/20November_1139/ckpt/46_91.1731.pt'
# CKPT_PATH = '/home/nas/user/minseok/FINAL_CONCAT_CYCLE/2/21November_0301/ckpt/best.pt'
...
CKPT_PATH = '/home/nas/user/minseok/FINAL_CONCAT_CYCLE/2/21November_0149/ckpt/45_90.1278.pt'
